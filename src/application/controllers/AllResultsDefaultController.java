...
public class AllResultsDefaultController implements Initializable  {
	...
	@FXML public void handleHelpAbout(ActionEvent event) {
		try {
			  ...
			  desktop.browse(oURL);
			  logger.info("Help > about clicked in AllResultsDefaultController!");
			} catch (Exception e) {
			  e.printStackTrace();
	...
	@FXML public void handleHyperlink(ActionEvent event) {
		try {
			  ...
			  desktop.browse(oURL);
			  logger.info("Hyperlink clicked in AllResultsDefaultController");
			} catch (Exception e) {
			  e.printStackTrace();
			  logger.severe("Unable to handle hyperlink in AllResultsDefaultController: " + e.getMessage());
			}
	}
	...
	@FXML public void handleShowHideButton(ActionEvent event) {
		try {
			Parent mainViewParent = FXMLLoader.load(getClass().getResource("/application/views/MainDefault.fxml"));
			...
			e.printStackTrace();
			logger.severe("Unable to switch to MainDefault.fxml from AllResultsDefaultController: " + e.getMessage());
		}
